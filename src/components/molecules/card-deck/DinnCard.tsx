...
export const DinnCard = ({ model, back, flipAction }: DinnCardInterface) => {
  ...
  const renderFront = () => {
    return (
      <View style={s.containerFront} onLayout={onLayout}>
        <Button
          title={'flip'}
  ...
  const renderBack = () => {
    return (
      <View style={[s.containerBack, { backgroundColor: primaryColor }]} onLayout={onLayout}>
        ...
        <Text style={[s.text, { color: accent1Color }]}>{'back'}</Text>
        <Button
          title={'test'}
          titleStyle={theme.styles.buttonScreenHeaderTitle}
          buttonStyle={theme.styles.buttonScreenHeader}
  ...
  return <>{back ? renderBack() : renderFront()}</>;
};
