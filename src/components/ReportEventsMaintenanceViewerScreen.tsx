...
export type Props = NativeStackScreenProps<ReportViewerNavigatorParamList, 'ReportEventsMaintenanceViewer'>;
...
const ReportEventsMaintenanceViewerScreen = ({ route, navigation }: Props) => {
  ...
  const events = useQuery<Event>('Event', event => {
    return event.filtered(
      `model ${queryRelationMap[emValues.model.relation]} $0` + ' AND ' +
      `modelType ${queryRelationMap[emValues.modelType.relation]} $1` + ' AND ' +
      `category ${queryRelationMap[emValues.category.relation]} $2` + ' AND ' +
      `date ${queryRelationMap[emValues.date.relation]} $3` + ' AND ' +
      `duration ${queryRelationMap[emValues.duration.relation]} $4` + ' AND ' +
      `pilot ${queryRelationMap[emValues.pilot.relation]} $5` + ' AND ' +
      `location ${queryRelationMap[emValues.location.relation]} $6` + ' AND ' +
      `modelStyle ${queryRelationMap[emValues.modelStyle.relation]} $7` + ' AND ' +
  ...
  return (
    <ViewShot
      options={{ fileName: 'em-report', format: 'png', quality: 0.9 }}
      style={s.container}>
