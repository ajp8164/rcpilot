...
export const useBatteryCyclesFilter = (batteryId: string) => {
  ...
  if (filter.dischargeDate.relation !== DateRelation.Any) {
    result = result.filtered(`discharge.date ${RQL[filter.dischargeDate.relation]} $0`, getDate(filter.dischargeDate));
  }
  if (filter.dischargeDuration.relation !== NumberRelation.Any) {
    result = result.filtered(`discharge.duration ${RQL[filter.dischargeDuration.relation]} $0`, MSSToSeconds(filter.dischargeDuration.value[0]));
  }
  if (filter.chargeDate.relation !== DateRelation.Any) {
    result = result.filtered(`charge.date ${RQL[filter.chargeDate.relation]} $0`, getDate(filter.chargeDate));
  }
  if (filter.chargeAmount.relation !== NumberRelation.Any) {
    result = result.filtered(`charge.amount ${RQL[filter.chargeAmount.relation]} $0`, MSSToSeconds(filter.chargeAmount.value[0]));
  }
