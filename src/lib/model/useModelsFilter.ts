...
export const useModelsFilter = () => {
  ...
  if (filter.modelType.relation !== EnumRelation.Any) {
    result = result.filtered(`type ${RQL[filter.modelType.relation]} $0`, [
      ...filter.modelType.value,
    ]);
  }
  if (filter.lastEvent.relation !== DateRelation.Any) {
    result = result.filtered(
      `lastEvent ${RQL[filter.lastEvent.relation]} $0`,
      getDate(filter.lastEvent),
    );
  }
  if (filter.vendor.relation !== StringRelation.Any) {
    result = result.filtered(
      `vendor TEXT $0`,
      `${RQL[filter.notes.relation]}${filter.vendor.value[0]}`,
    );
  }
